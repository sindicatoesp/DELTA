...
export async function handleApprovalWorkflowService(
	...
	if (!userCountryAccounts) {
		throw new Error(
			"User does not have access to the specified country account",
		);
	}
			...
			switch (action) {
				case "submit-validation":
					if (!validatorUserIds || validatorUserIds.length === 0) {
						throw new Error(
							"Validator user IDs are required for submit-validation action",
						);
					}
...
async function handleSubmitForValidation(
	...
	const validationAssignedData: EntityValidationAssignmentFields[] =
		validatorUserIds.map((uuidValidatorAssignedTo) => ({
			entityId,
			entityType,
