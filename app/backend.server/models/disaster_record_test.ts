...
describe("Disaster Record Tenant Isolation Tests", async () => {
	...
	it("should create disaster records with tenant isolation", async () => {
		...
		assert.strictEqual(
			result3.ok,
			false,
			"Disaster record creation with cross-tenant disaster event should fail",
		);
		...
		assert.strictEqual(
			crossTenantAccess1,
			null,
			"Tenant 1 should NOT be able to access tenant 2's disaster record",
		);
	...
	it("should enforce tenant isolation when updating disaster records", async () => {
		...
		assert.strictEqual(
			updateResult1.ok,
			true,
			"Tenant 1 should be able to update their own disaster record",
		);
		...
		assert.strictEqual(
			updateResult2.ok,
			false,
			"Tenant 2 should not be able to update tenant 1's disaster record",
		);
	});
	...
	it("should enforce tenant isolation when deleting disaster records", async () => {
		...
		assert.strictEqual(
			deleteResult1.ok,
			false,
			"Tenant 2 should not be able to delete tenant 1's disaster record",
		);
		...
		assert.strictEqual(
			deleteResult2.ok,
			true,
			"Tenant 1 should be able to delete their own disaster record",
		);
