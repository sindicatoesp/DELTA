...
export async function getFictitiousCountryById(
	id: string,
): Promise<SelectCountries | null> {
...
export async function getFictitiousCountriesOrderByName() {
	return CountryRepository.getByTypeOrderByName(COUNTRY_TYPE.FICTIONAL);
}

export async function createFictitiousCountry(
	nameInput: string,
): Promise<void> {
...
export async function updateFictitiousCountry(
	id: string,
	nameInput: string,
): Promise<void> {
	const name = nameInput.trim();
...
export async function deleteFictitiousCountry(id: string): Promise<void> {
	const existing = await getFictitiousCountryById(id);
	if (!existing) {
		throw new FictitiousCountryNotFoundError();
	}
