...
const Filters: React.FC<FiltersProps> = ({ onApplyFilters, onClearFilters, sectorsData, geographicLevelsData, disasterEventsData, hazardTypesData: hazardTypesDataProp, hazardClustersData: hazardClustersDataProp, specificHazardsData: specificHazardsDataProp }) => {
			...
			try {
				...
				if (disasterEventsData) {
					if (Array.isArray(disasterEventsData)) {
						...
						disasterEventsData.rows &&
						Array.isArray(disasterEventsData.rows)
					) {
						...
						if (Array.isArray(disasterEventsData.disasterEvents)) {
							...
							disasterEventsData.disasterEvents.rows &&
							Array.isArray(disasterEventsData.disasterEvents.rows)
						) {
							setDisasterEvents(disasterEventsData.disasterEvents.rows);
						} else {
							console.warn(
								"Disaster events data structure not recognized:",
								disasterEventsData,
							);
							setDisasterEvents([]);
						}
	...
	const [hazardTypesData, setHazardTypesData] =
		useState<HazardTypesResponse | null>(null);
	...
	const [hazardClustersData, setHazardClustersData] =
		useState<HazardClustersResponse>({ hazardClusters: [] });
			...
			const processHazardClustersData = async () => {
				try {
					...
					if (hazardClustersDataProp?.clusters) {
						...
						typeof hazardClustersDataProp === "object" &&
						hazardClustersDataProp !== null
					) {
						...
						const arrayProps = Object.entries(hazardClustersDataProp).find(
	...
	const [specificHazardsData, setSpecificHazardsData] =
		useState<SpecificHazardsResponse>({ hazards: [] });
			...
			const processSpecificHazardsData = async () => {
				try {
					...
					if (specificHazardsDataProp?.hazards) {
						...
						specificHazardsDataProp &&
						typeof specificHazardsDataProp === "object"
					) {
						// If no array property is found, try to use the object itself if it looks like an array of objects
					...
					if (searchQuery) {
						...
						searchFilteredHazards = filteredHazards.filter((hazard: any) =>
							hazard.name.toLowerCase().includes(lowerCaseQuery),
						);
					}
	...
	const handleSpecificHazardSelection = async (specificHazardId: string) => {
		try {
			...
			if (specificHazardsData && specificHazardsData.hazards) {
				...
				if (
					selectedHazard &&
					selectedHazard.hazardClusterId &&
					selectedHazard.hazardTypeId
				) {
	...
	const geographicLevels: Hazard[] =
		geographicLevelsData?.levels.map(
			(level: { id: number; name: { en: string } }) => ({
	...
	const filteredEvents = React.useMemo(() => {
		...
		if (endTime - startTime > 50) {
			// Only log if search took more than 50ms
		}
	...
	const handleFilterChange = (field: keyof typeof filters, value: string) => {
		...
		setFilters((prev) => {
			...
			if (
				["hazardTypeId", "hazardClusterId", "specificHazardId"].includes(field)
			) {
				...
				>
					...
					{(() => {
						const selectedSector = processedSectors.find(
							(s: Sector) => s.id.toString() === filters.sectorId,
						);
