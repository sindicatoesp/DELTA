	...
	(
			...
			return nodes
				.map((node: any): any | null => {
					...
					const filteredChildren =
						filterTree(node.children, query, expandedState) || [];
		...
		const handleCheckboxChange = (e: any) => {
			...
			setCheckedItems((prev) => {
				const newState = {
					...prev,
					[e.target.closest("li").getAttribute("data-id")]: e.target.checked,
				};
				return newState;
			});
		};
				...
				{nodes.map((node) => {
					...
					return (
						<li
							...
							data-ids={enrichedNode.ids}
							data-path={enrichedNode.path}
							data-has_children={enrichedNode.has_children}
						>
							{enrichedNode.children.length > 0 ? (
								<>
									...
									{Object.entries(enrichedNode.hiddenData || {}).map(
										([field, value]) => (
											<textarea
												data-id={enrichedNode.id}
		...
		return (
			<>
				{dialogMode ? (
					...
					<div ref={dialogRef}>
						{treeViewContent(showActionFooter || false)}
					</div>
				)}
				{disableButtonSelect ? null : (
					<button className="tree-button-select" onClick={treeViewOpen}>
						{caption}
					</button>
				)}
			</>
...
export const buildTree = (
	...
	list.forEach((item) => {
		...
		if (typeof nameValue !== "object") {
			...
			const validPriorityKey =
				typeof priorityKey === "string" && priorityKey.trim() !== ""
					? priorityKey
					: null;
