...
const convertToTreeNodes = (items: SectorListItem[]) => {
    const map = new Map();
    const roots: any[] = [];
...
export default function SectorsPage({ sectors, userRole }: SectorsPageProps) {
    ...
    const flattenedNodes = useMemo(() => {
        ...
        const stack: Array<{ node: any; depth: number }> = treeNodes
    ...
    const filteredNodes = useMemo(() => {
        ...
        return flattenedNodes.filter((item) =>
            nodesToInclude.has(item.node.key as string),
        );
    ...
    const descriptionTemplate = useCallback(
        (rowData: SectorListItem) => (
            <div className="whitespace-pre-wrap text-sm">{rowData.description || "-"}</div>
        ),
        [],
    );

    const parentTemplate = useCallback(
        (rowData: SectorListItem) => (
            <span className="text-sm">{rowData.parentId ? rowData.parentName : "None"}</span>
        ),
        [],
    );
