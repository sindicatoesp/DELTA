...
export class DrizzleCountryAccountRepository implements CountryAccountRepositoryPort {
	constructor(private readonly db: CountryAccountDb) {}
	...
	async resendInvitation(countryAccountId: string) {
		const countryAccount = await this.db.query.countryAccountsTable.findFirst({
